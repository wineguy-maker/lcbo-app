...
def load_food_items():
    try:
        food_items = pd.read_csv('food_items.csv') # Ensure this file is deployed
        return food_items
    except Exception as e:
...
def sort_data_filter(data, sort_by):
    ...
    df_sorted = data.copy()
    ascending = True; sort_column = None

    if sort_by == '# of reviews': sort_column = 'raw_avg_reviews'; ascending = False
    elif sort_by == 'Rating': sort_column = 'raw_ec_rating'; ascending = False
    elif sort_by == 'Top Viewed - Year': sort_column = 'raw_view_rank_yearly'
    elif sort_by == 'Top Viewed - Month': sort_column = 'raw_view_rank_monthly'
    elif sort_by == 'Top Seller - Year': sort_column = 'raw_sell_rank_yearly'
    elif sort_by == 'Top Seller - Month': sort_column = 'raw_sell_rank_monthly'
    else: sort_column = 'weighted_rating'; ascending = False

    if sort_column and sort_column in df_sorted.columns:
        if sort_column in ['raw_avg_reviews', 'raw_ec_rating', 'weighted_rating', 'raw_view_rank_yearly', 'raw_view_rank_monthly', 'raw_sell_rank_yearly', 'raw_sell_rank_monthly']:
             df_sorted[sort_column] = pd.to_numeric(df_sorted[sort_column], errors='coerce')
        try: return df_sorted.sort_values(by=sort_column, ascending=ascending, na_position='last')
        except Exception as e: st.warning(f"Could not sort by {sort_column}: {e}. Returning unsorted."); return data
    elif sort_by != 'Sort by': st.warning(f"Could not sort by selected option '{sort_by}'."); return data
    else: # Default sort if 'Sort by' selected
         if 'weighted_rating' in df_sorted.columns: df_sorted['weighted_rating'] = pd.to_numeric(df_sorted['weighted_rating'], errors='coerce'); return df_sorted.sort_values(by='weighted_rating', ascending=False, na_position='last')
         else: return data
...
def filter_data(data, country='All Countries', region='All Regions', varietal='All Varietals', exclude_usa=False, in_stock=False, only_vintages=False, store='Select Store'):
    ...
    df = data.copy()
    try:
        if country != 'All Countries' and 'raw_country_of_manufacture' in df.columns: df = df[df['raw_country_of_manufacture'] == country]
        if region != 'All Regions' and 'raw_lcbo_region_name' in df.columns: df = df[df['raw_lcbo_region_name'] == region]
        if varietal != 'All Varietals' and 'raw_lcbo_varietal_name' in df.columns: df = df[df['raw_lcbo_varietal_name'] == varietal]
        if store != 'Select Store' and 'store_name' in df.columns: df = df[df['store_name'] == store]
        if in_stock and 'stores_inventory' in df.columns: df['stores_inventory'] = pd.to_numeric(df['stores_inventory'], errors='coerce').fillna(0); df = df[df['stores_inventory'] > 0]
        if only_vintages and 'raw_lcbo_program' in df.columns: df = df[df['raw_lcbo_program'].astype(str).str.contains(r"['\"]Vintages['\"]", regex=True, na=False)]
        if exclude_usa and 'raw_country_of_manufacture' in df.columns: df = df[df['raw_country_of_manufacture'] != 'United States']
    except Exception as e: st.error(f"Error during filtering step: {e}"); return data
    return df
...
def load_favourites():
    ...
    try:
        ...
        if hasattr(response, 'error') and response.error: st.error(f"Supabase error loading favourites: {response.error}"); return []
        return [record["URI"] for record in response.data]
    except Exception as e: st.error(f"Error loading favourites: {e}"); return []


def save_favourites(favourites_to_add):
    """Save a list of favourite URIs to Supabase for the 'admin' user."""
    user_id_to_save = "admin"; today_str = datetime.now().strftime("%Y-%m-%d")
    records_to_upsert = [{"URI": uri, "Date": today_str, "User ID": user_id_to_save} for uri in favourites_to_add]
    if records_to_upsert:
        try:
            response = supabase.table(FAVOURITES_TABLE).upsert(records_to_upsert).execute()
            if hasattr(response, 'error') and response.error: st.error(f"Supabase error saving favourites: {response.error}")
            else: st.success("Favourites saved successfully!"); st.session_state.favourites = load_favourites(); st.rerun()
        except Exception as e: st.error(f"Error saving favourites: {e}")


def delete_favourites(favourites_to_delete):
    """Remove a list of favourite URIs from Supabase for the 'admin' user."""
    user_id_to_delete = "admin"; success = True
    for uri in favourites_to_delete:
        deleted_data = supabase_delete_record(FAVOURITES_TABLE, uri, user_id_to_delete)
        if deleted_data is None: success = False
    if success and favourites_to_delete: st.success("Favourites deleted successfully!"); st.session_state.favourites = load_favourites(); st.rerun()


def toggle_favourite(wine_uri):
    """Toggle the favourite status of a wine for the 'admin' user."""
    current_favourites = load_favourites()
    if wine_uri in current_favourites: delete_favourites([wine_uri])
    else: save_favourites([wine_uri])
...
def transform_image_url(url, new_size):
    ...
    try: return re.sub(r"\d+\.\d+\.(png|jpg|jpeg)$", new_size, url, flags=re.IGNORECASE)
...
def get_favourites_with_lowest_price():
    ...
    if not favourite_uris: st.info("No favourites found for user 'admin'."); return []
    all_products = supabase_get_records(PRODUCTS_TABLE)
    if not all_products: st.warning("Could not load product data to check prices."); return []
    products_df = pd.DataFrame(all_products); products_df = products_df[products_df['uri'].isin(favourite_uris)]
    if products_df.empty: st.info("No product data found for the favourited items."); return []
    price_history = supabase_get_records(PRICE_HISTORY_TABLE)
    if not price_history: st.warning("Could not load price history data."); return []
    price_history_df = pd.DataFrame(price_history); price_history_df = price_history_df[price_history_df['URI'].isin(favourite_uris)]
    lowest_price_items = []
    if not price_history_df.empty: price_history_df['Price'] = pd.to_numeric(price_history_df['Price'], errors='coerce'); lowest_prices = price_history_df.loc[price_history_df.groupby('URI')['Price'].idxmin()]
    else: lowest_prices = pd.DataFrame(columns=['URI', 'Price'])
    for _, product in products_df.iterrows():
        uri = product.get("uri");
        if not uri: continue
        current_price_str = product.get("raw_ec_promo_price")
        if pd.isna(current_price_str) or str(current_price_str).strip() == '' or str(current_price_str).strip().upper() == 'N/A': current_price_str = product.get("raw_ec_price")
        current_price_numeric = pd.to_numeric(current_price_str, errors='coerce')
        if pd.isna(current_price_numeric): continue
        lowest_entry = lowest_prices[lowest_prices['URI'] == uri]
        if not lowest_entry.empty:
            lowest_price_numeric = lowest_entry['Price'].iloc[0]
            if pd.isna(lowest_price_numeric): continue
            if current_price_numeric <= lowest_price_numeric: lowest_price_items.append({"Title": product.get("title", "Unknown Title"), "URI": uri, "Current Price": f"${current_price_numeric:.2f}", "Lowest Price": f"${lowest_price_numeric:.2f}"})
...
def send_email_with_lowest_prices(items):
    ...
    if not items: st.info("No items at lowest price to report. Email not sent."); return
    smtp_server = "smtp-broadcasts.postmarkapp.com"; smtp_port = 587
    smtp_username = "PM-B-broadcast-o5E13wA0FjsIeCQNnCbh3"; smtp_password = "_PShFYnMnyik9CCoMZi7cog6W_oV8PjnxsK7" # Hardcoded
    sender_email = "winefind@justemail.ca"; receiver_email = "winefind@justemail.ca" # Hardcoded
    subject = "Favourites at Their Lowest Price Alert!"; message = MIMEMultipart("alternative")
    message["From"] = sender_email; message["To"] = receiver_email; message["Subject"] = subject
    ...
    for item in items:
        ...
        title_safe = item['Title'].replace('<', '&lt;').replace('>', '&gt;'); uri_safe = item['URI'].replace('<', '&lt;').replace('>', '&gt;')
        current_price_safe = item['Current Price'].replace('<', '&lt;').replace('>', '&gt;'); lowest_price_safe = item['Lowest Price'].replace('<', '&lt;').replace('>', '&gt;')
    ...
    html_content += "</tbody></table></body></html>"; message.attach(MIMEText(text_content, "plain")); message.attach(MIMEText(html_content, "html"))
    try:
        st.info(f"Attempting to send email via {smtp_server}:{smtp_port}...")
        with smtplib.SMTP(smtp_server, smtp_port) as server: server.ehlo(); server.starttls(); server.ehlo(); server.login(smtp_username, smtp_password); server.sendmail(sender_email, receiver_email, message.as_string())
        st.success("Email sent successfully!")
    except smtplib.SMTPAuthenticationError: st.error("SMTP Authentication Error: Incorrect username or password (using hardcoded values).")
    except smtplib.SMTPConnectError: st.error(f"SMTP Connection Error: Failed to connect to {smtp_server}:{smtp_port}.")
    except Exception as e: st.error(f"Failed to send email: {e}")
# --- End of reverted SMTP function ---
...
def background_update(products_list, today_str):
    ...
    for product in products_list:
        product_record = product.copy(); product_record["Date"] = today_str
        if 'uri' not in product_record or pd.isna(product_record['uri']): continue
        records_to_upsert.append(product_record)
    if records_to_upsert:
         st.info(f"Upserting {len(records_to_upsert)} products to Supabase..."); batch_size = 400
         for i in range(0, len(records_to_upsert), batch_size):
             batch = records_to_upsert[i:i + batch_size]; upsert_result = supabase_upsert_record(PRODUCTS_TABLE, batch)
    ...
    for product in products_list:
         uri = product.get('uri'); price_str = product.get("raw_ec_promo_price")
         if pd.isna(price_str) or str(price_str).strip() == '' or str(price_str).strip().upper() == 'N/A': price_str = product.get("raw_ec_price")
         price_numeric = pd.to_numeric(price_str, errors='coerce')
         if uri and not pd.isna(price_numeric): price_history_records.append({"URI": uri, "Price": price_numeric, "Date": today_str})
    if price_history_records:
         st.info(f"Adding {len(price_history_records)} price history records..."); response = supabase.table(PRICE_HISTORY_TABLE).upsert(price_history_records).execute()
    ...
    # 3. Check favourites and send email
    lowest_price_items = get_favourites_with_lowest_price(); send_email_with_lowest_prices(lowest_price_items)
...
def refresh_data(store_id=None):
    ...
    current_time = datetime.now(); today_str = current_time.strftime("%Y-%m-%d")
    ...
    # --- Using Hardcoded API Token ---
    # WARNING: Hardcoding API tokens is a security risk.
    headers = { "User-Agent": "Mozilla/5.0 StreamlitApp/1.1", "Accept": "application/json", "Authorization": "Bearer xx883b5583-07fb-416b-874b-77cce565d927", "Content-Type": "application/json", "Referer": "https://www.lcbo.com/" }
    # --- End Hardcoded Token ---
    ...
    if store_id: dictionaryFieldContext = { "stores_inventory": store_id, "stores_stock_combined": store_id, "stores_low_stock_combined": store_id }; base_payload["dictionaryFieldContext"] = dictionaryFieldContext
    def get_items(payload):
        try: response = requests.post(url, headers=headers, json=payload, timeout=30); response.raise_for_status(); return response.json()
        except requests.exceptions.RequestException as e: st.error(f"API request failed: {e}"); return None
        except Exception as e: st.error(f"Error processing API response: {e}"); return None
    st.info("Fetching initial batch from API..."); initial_data = get_items(base_payload)
    if not (initial_data and 'results' in initial_data): st.error("Failed to retrieve initial data from the API."); st.error(f"API Response: {initial_data}"); return None
    all_items = initial_data['results']; total_count = initial_data.get('totalCount', 0); st.info(f"API reported total count: {total_count}")
    num_requests = (total_count // 500) + (1 if total_count % 500 != 0 else 0); st.info(f"Fetching {num_requests} batches..."); progress_bar = st.progress(0)
    for i in range(1, num_requests):
        current_payload = base_payload.copy(); current_payload["firstResult"] = i * 500
        ...
        progress = int(((i + 1) / num_requests) * 100); progress_bar.progress(progress); time.sleep(0.5)
    progress_bar.empty(); st.info(f"Total items fetched: {len(all_items)}")
    # --- START: Weighted Rating Calculation ---
    temp_df = pd.DataFrame([{'raw_ec_rating': item['raw'].get('ec_rating'), 'raw_avg_reviews': item['raw'].get('avg_reviews')} for item in all_items])
    temp_df['numeric_rating'] = pd.to_numeric(temp_df['raw_ec_rating'], errors='coerce'); temp_df['numeric_reviews'] = pd.to_numeric(temp_df['raw_avg_reviews'], errors='coerce')
    valid_reviewed_items = temp_df.dropna(subset=['numeric_reviews', 'numeric_rating']); valid_reviewed_items = valid_reviewed_items[valid_reviewed_items['numeric_reviews'] > 0]
    ...
    st.info(f"Calculated mean_rating (C) for weighted score: {mean_rating:.4f}"); st.info(f"Using fixed minimum_votes (m) for weighted score: {minimum_votes}")
    def weighted_rating(rating, votes, min_votes, avg_rating):
        try: rating = float(rating) if pd.notna(rating) else 0.0; votes = float(votes) if pd.notna(votes) else 0.0; min_votes = float(min_votes); avg_rating = float(avg_rating)
    ...
    for product_api_item in all_items:
        raw_data = product_api_item['raw']; rating_val = raw_data.get('ec_rating'); votes_val = raw_data.get('avg_reviews')
        product_info = { 'title': product_api_item.get('title', 'N/A'), 'uri': product_api_item.get('uri', 'N/A'), 'raw_ec_thumbnails': raw_data.get('ec_thumbnails', 'N/A'), 'raw_ec_shortdesc': raw_data.get('ec_shortdesc', 'N/A'), 'raw_lcbo_tastingnotes': raw_data.get('lcbo_tastingnotes', 'N/A'), 'raw_lcbo_region_name': raw_data.get('lcbo_region_name', 'N/A'), 'raw_country_of_manufacture': raw_data.get('country_of_manufacture', 'N/A'), 'raw_lcbo_program': raw_data.get('lcbo_program', 'N/A'), 'raw_created_at': raw_data.get('created_at', 'N/A'), 'raw_is_buyable': raw_data.get('is_buyable', 'N/A'), 'raw_ec_price': raw_data.get('ec_price', 'N/A'), 'raw_ec_final_price': raw_data.get('ec_final_price', 'N/A'), 'raw_ec_promo_price': raw_data.get('ec_promo_price', 'N/A'), 'raw_lcbo_unit_volume': raw_data.get('lcbo_unit_volume', 'N/A'), 'raw_lcbo_alcohol_percent': raw_data.get('lcbo_alcohol_percent', 'N/A'), 'raw_lcbo_sugar_gm_per_ltr': raw_data.get('lcbo_sugar_gm_per_ltr', 'N/A'), 'raw_lcbo_bottles_per_pack': raw_data.get('lcbo_bottles_per_pack', 'N/A'), 'raw_sysconcepts': raw_data.get('sysconcepts', 'N/A'), 'raw_ec_category': raw_data.get('ec_category', 'N/A'), 'raw_ec_category_filter': raw_data.get('ec_category_filter', 'N/A'), 'raw_lcbo_varietal_name': raw_data.get('lcbo_varietal_name', 'N/A'), 'raw_stores_stock': raw_data.get('stores_stock', 'N/A'), 'raw_stores_stock_combined': raw_data.get('stores_stock_combined', 'N/A'), 'raw_stores_low_stock_combined': raw_data.get('stores_low_stock_combined', 'N/A'), 'raw_stores_low_stock': raw_data.get('stores_low_stock', 'N/A'), 'raw_out_of_stock': raw_data.get('out_of_stock', 'N/A'), 'stores_inventory': raw_data.get('stores_inventory', 0), 'raw_online_inventory': raw_data.get('online_inventory', 0), 'raw_avg_reviews': votes_val, 'raw_ec_rating': rating_val, 'weighted_rating': weighted_rating(rating_val, votes_val, minimum_votes, mean_rating), 'raw_view_rank_yearly': raw_data.get('view_rank_yearly', 'N/A'), 'raw_view_rank_monthly': raw_data.get('view_rank_monthly', 'N/A'), 'raw_sell_rank_yearly': raw_data.get('sell_rank_yearly', 'N/A'), 'raw_sell_rank_monthly': raw_data.get('sell_rank_monthly', 'N/A')}
    ...
    thread = threading.Thread(target=background_update, args=(products_list_for_update, today_str), daemon=True); thread.start()
...
def main():
    ...
    with st.sidebar.expander("Admin Authorization", expanded=not st.session_state.authorized):
        ...
        if st.button("Login", key="auth_submit"):
            correct_pin = st.secrets.get("correct_pin", None) # Fetch PIN from secrets
            if correct_pin and pin_input == correct_pin: st.session_state.authorized = True; st.rerun()
            else: st.sidebar.error("Incorrect PIN.")

    # Session State Init
    if "favourites" not in st.session_state: st.session_state.favourites = load_favourites() if st.session_state.authorized else []
    ...
    selected_store = st.sidebar.selectbox("Select Store", options=store_options, key="store_selector", index=store_options.index(st.session_state.selected_store))

    # Data Loading / Refresh Logic
    needs_refresh = False
    if selected_store != st.session_state.selected_store: st.info(f"Store changed to {selected_store}. Triggering data refresh."); st.session_state.selected_store = selected_store; needs_refresh = True
    if st.sidebar.button("Refresh Data from API", key="manual_refresh"): needs_refresh = True; st.info("Manual refresh requested.")
    if needs_refresh:
        ...
        elif refreshed_data is not None and refreshed_data.empty: st.session_state.data = refreshed_data; st.warning("API returned no data for selection.")
    ...
    categories = food_items['Category'].unique() if not food_items.empty else []; food_options = ['All Dishes'] + sorted(categories.tolist())
    country = st.sidebar.selectbox("Country", options=country_options); region = st.sidebar.selectbox("Region", options=region_options); varietal = st.sidebar.selectbox("Varietal", options=varietal_options)
    food_category = st.sidebar.selectbox("Food Category", options=food_options); exclude_usa = st.sidebar.checkbox("Exclude USA", value=False); in_stock = st.sidebar.checkbox("In Stock Only (Selected Store)", value=False)
    only_vintages = st.sidebar.checkbox("Only Vintages", value=False); only_sale_items = st.sidebar.checkbox("Only Sale Items", value=False); only_favourites = st.sidebar.checkbox("Only My Favourites", value=False) if st.session_state.authorized else False
    ...
    if not data_to_display.empty:
        ...
        filtered_data = filter_and_sort_data(data_to_display, sort_by, **filters) # Uses corrected function
        # Post-filtering
        if only_sale_items and 'raw_ec_promo_price' in filtered_data.columns: filtered_data['promo_price_numeric'] = pd.to_numeric(filtered_data['raw_ec_promo_price'], errors='coerce'); filtered_data = filtered_data[filtered_data['promo_price_numeric'].notna() & (filtered_data['promo_price_numeric'] > 0)]; filtered_data = filtered_data.drop(columns=['promo_price_numeric'])
        if only_favourites and st.session_state.authorized and 'uri' in filtered_data.columns: current_favs = st.session_state.favourites; filtered_data = filtered_data[filtered_data['uri'].isin(current_favs)]
        if food_category != 'All Dishes' and not food_items.empty and 'raw_sysconcepts' in filtered_data.columns:
            ...
            def check_food_match(sysconcepts_str):
                 if pd.isna(sysconcepts_str): return False; text_to_check = str(sysconcepts_str).lower(); return any(item in text_to_check for item in selected_food_items)
    ...
    if not filtered_data.empty:
        page_size = 10; total_products = len(filtered_data); total_pages = (total_products // page_size) + (1 if total_products % page_size > 0 else 0)
        if total_pages > 0: page = st.number_input("Page", min_value=1, max_value=total_pages, value=1, step=1, key="pagination")
        else: page = 1
        start_idx = (page - 1) * page_size; end_idx = start_idx + page_size; page_data = filtered_data.iloc[start_idx:end_idx]
        for idx, row in page_data.iterrows():
            ...
            with col2:
                ...
                with st.expander("More Details"):
                    st.markdown(f"**Title:** {row.get('title', 'N/A')}"); st.markdown(f"**URI:** {row.get('uri', 'N/A')}")
